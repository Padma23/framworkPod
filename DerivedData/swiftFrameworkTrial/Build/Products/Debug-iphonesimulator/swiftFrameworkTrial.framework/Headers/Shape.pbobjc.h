// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: shape.proto

// This CPP symbol can be defined to use imports that match up to the framework
// imports needed when using CocoaPods.
#if !defined(GPB_USE_PROTOBUF_FRAMEWORK_IMPORTS)
 #define GPB_USE_PROTOBUF_FRAMEWORK_IMPORTS 0
#endif

#if GPB_USE_PROTOBUF_FRAMEWORK_IMPORTS
 #import <Protobuf/GPBProtocolBuffers.h>
#else
 #import "GPBProtocolBuffers.h"
#endif

#if GOOGLE_PROTOBUF_OBJC_VERSION < 30002
#error This file was generated by a newer version of protoc which is incompatible with your Protocol Buffer library sources.
#endif
#if 30002 < GOOGLE_PROTOBUF_OBJC_MIN_SUPPORTED_VERSION
#error This file was generated by an older version of protoc which is incompatible with your Protocol Buffer library sources.
#endif

// @@protoc_insertion_point(imports)

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"

CF_EXTERN_C_BEGIN

@class EditorCompatible;
@class NonVisualShapeProps;
@class Properties;
@class TextBody;
@class ThemeReference;

NS_ASSUME_NONNULL_BEGIN

#pragma mark - ShapeRoot

/**
 * Exposes the extension registry for this file.
 *
 * The base class provides:
 * @code
 *   + (GPBExtensionRegistry *)extensionRegistry;
 * @endcode
 * which is a @c GPBExtensionRegistry that includes all the extensions defined by
 * this file and all files that it depends on.
 **/
@interface ShapeRoot : GPBRootObject
@end

#pragma mark - Shape

typedef GPB_ENUM(Shape_FieldNumber) {
  Shape_FieldNumber_NvOprops = 1,
  Shape_FieldNumber_Props = 2,
  Shape_FieldNumber_ThemeRef = 3,
  Shape_FieldNumber_TextBody = 4,
  Shape_FieldNumber_Edit = 5,
};

@interface Shape : GPBMessage

/** Non Visual Properties for Shape. */
@property(nonatomic, readwrite, strong, null_resettable) NonVisualShapeProps *nvOprops;
/** Test to see if @c nvOprops has been set. */
@property(nonatomic, readwrite) BOOL hasNvOprops;

/** Shape properties. */
@property(nonatomic, readwrite, strong, null_resettable) Properties *props;
/** Test to see if @c props has been set. */
@property(nonatomic, readwrite) BOOL hasProps;

/** Theme Style. */
@property(nonatomic, readwrite, strong, null_resettable) ThemeReference *themeRef;
/** Test to see if @c themeRef has been set. */
@property(nonatomic, readwrite) BOOL hasThemeRef;

/** Text body */
@property(nonatomic, readwrite, strong, null_resettable) TextBody *textBody;
/** Test to see if @c textBody has been set. */
@property(nonatomic, readwrite) BOOL hasTextBody;

/** Editor compatible attributes. */
@property(nonatomic, readwrite, strong, null_resettable) EditorCompatible *edit;
/** Test to see if @c edit has been set. */
@property(nonatomic, readwrite) BOOL hasEdit;

@end

NS_ASSUME_NONNULL_END

CF_EXTERN_C_END

#pragma clang diagnostic pop

// @@protoc_insertion_point(global_scope)
